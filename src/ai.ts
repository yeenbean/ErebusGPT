...
export const ErebusDoThingy = {
    ...
    performModeration: async (userMessage: string): Promise<number> => {
        // Check moderation
        let result = await ai.createModeration({
            input: userMessage
        });
        ...
        // Interpret response
        if (result.data.results[0].flagged) { return 1 } else { return 0 };
    },
    ...
    getResponse: async (userMessage: string): Promise<string> => {
        ...
        // typescript...
        if (!result) return 'oops, didnt get that, mind giving it another go? (error -1)';
        if (!result.data.choices[0].message) return 'oops my bad i didnt get that. mind giving it another shot? (error -2) 😅';
    
        // set the response to a simple string (its easier trust me)
        let response = result.data.choices[0].message.content;  // bffr
        if (!response) return 'oops my bad i didnt get that. mind giving it another shot? (error -3) 😅';
